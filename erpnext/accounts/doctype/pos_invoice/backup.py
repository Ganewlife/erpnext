...
class POSInvoice(SalesInvoice):
	...
	def update_patient_consumption(doc, method):
		if doc.status == "Paid":
			patient_name = doc.customer_name
			total_assureur = doc.total_assureur
			...
			if patient:
				...
				nouveau_montant_consommé = patient.montant_consommé + total_assureur

				# Vérifier si le nouveau montant consommé dépasse le plafond de couverture
				if nouveau_montant_consommé > patient.plafond_de_couverture:
					nouveau_montant_consommé = patient.plafond_de_couverture
					frappe.msgprint(f"Le montant consommé atteint le plafond de couverture pour le patient {patient_name}.")

				# Mettre à jour le champ montant_consommé du patient
				patient.montant_consommé = nouveau_montant_consommé
				patient.save()
 """
