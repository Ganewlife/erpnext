erpnext.PointOfSale.ItemCart = class {
			...
			`<div class="cart-container">
				<div class="abs-cart-container">
					...
					<div class="cart-header">
						<div class="name-header"style="text-align: justify;">${__("Item")}</div>
						<div class="qty-header"style="margin-left: 2px; width: 70px; text-align: justify; ">${__("Quantity")}</div>
						<div class="rate-amount-header"style="text-align: justify;">${__("Amount")}</div>
						<div class="custom-part-assure-header"style="margin-right: 15px; width: 80px; text-align: justify;">${__("Part assuré")}</div>
					</div>
	...
	/* make_cart_items_section() {
		...
		this.$cart_header.append(`<div class="custom-part-assureur-header">${__("Part assureur")}</div>`);
	...
	make_no_items_placeholder() {
		...
		this.$cart_items_wrapper.html(`<div class="no-item-wrapper">${__("Aucun article dans le panier")}</div>`);
	}
	...
	make_cart_totals_section() {
		...
		this.$totals_section.append(
			...
			<div class="part-assure-total-container"  style="display: flex; justify-content: space-between; width: 100%;">
			<div class="part-assure-label">${__("Total Part Assuré")}</div>
			<div class="part-assure-value net-total-value">0.00</div>
        	</div>
			
			<div class="part-assureur-total-container" style="display: flex; justify-content: space-between; width: 100%;">
			<div class="part-assureur-label">${__("Total Part Assureur")}</div>
			<div class="part-assureur-value net-total-value">0.00</div>
        	</div>

			<div class="net-total-container">
				<div class="net-total-label">${__("Total TTC")}</div>
...
/* 	make_customer_selector() {
	...
	make_customer_selector() {
		this.$customer_section.html(`
			<div class="customer-field"></div>
			<div class="subscription-field"></div>
		`);
		const me = this;
		const allowed_customer_group = this.allowed_customer_groups || [];
		let filters = {};
				...
				onchange: function () {
					if (this.value) {
						...
						frm.script_manager.trigger("customer", frm.doc.doctype, frm.doc.name).then(() => {
							frappe.run_serially([
								() => me.fetch_customer_details(this.value),
								() => me.fetch_subscriptions(this.value),
								() => me.events.customer_details_updated(me.customer_info),
								() => me.update_customer_section(),
								() => me.update_totals_section(),
								() => frappe.dom.unfreeze(),
		...
		this.subscription_field = frappe.ui.form.make_control({
			df: {
				label: __("Subscription"),
				fieldtype: "Select",
				options: [],
				placeholder: __("Select a subscription"),
			},
			parent: this.$customer_section.find(".subscription-field"),
			render_input: true,
		});
		this.subscription_field.toggle_label(false);
	}
	...
	// requete de la souscription
	fetch_subscriptions(customer) {
		if (customer) {
			return new Promise((resolve) => {
				frappe.call({
					method: "erpnext.selling.page.point_of_sale.point_of_sale.get_subscription_list",
					args: { customer: customer },
					callback: (r) => {
						if (r.message) {
							const options = r.message.map(sub => sub.fiche_de_souscription_assurance_sante);
							this.subscription_field.df.options = options.join("\n");
							this.subscription_field.refresh();
							resolve();
						}
					},
				});
			...
			return new Promise((resolve) => {
				this.subscription_field.df.options = [];
				this.subscription_field.refresh();
				resolve();
			});
		}
			...
			df: {
				...
				onchange: function () {
					if (flt(this.value) != 0) {
						...
						me.$add_discount_elem.html(`${me.get_discount_icon()} ${__("Ajouter remise")}`);
	...
	update_totals_section(frm) {
		...
		// this.render_net_total(part_assure_total);
		
		this.render_total_item_qty(frm.doc.items);
		
		const part_assure_total = frm.doc.items.reduce((total, item) => total + (Math.round(item.qty * item.custom_part_assuré)), 0);
		this.render_part_assure_total(part_assure_total);

		const part_assureur_total = frm.doc.items.reduce((total, item) => total + (Math.round(item.qty * item.custom_part_assureur)), 0);
		this.render_part_assureur_total(part_assureur_total);
		
		// const grand_total = cint(frappe.sys_defaults.disable_rounded_total)? frm.doc.grand_total: frm.doc.rounded_total;
		const grand_total = cint(frappe.sys_defaults.disable_rounded_total)? frm.doc.grand_total: frm.doc.rounded_total;
		// this.render_grand_total(grand_total);
		this.render_net_total(grand_total); // ttc
		this.render_grand_total(part_assure_total);
	...
	render_net_total(value) {
		...
		this.$totals_section
			...
			.html(`<div>${__("Total TTC")}</div><div>${format_currency(value, currency)}</div>`);

		this.$numpad_section
			.find(".numpad-net-total")
			.html(`<div>${__("Total TTC")}: <span>${format_currency(value, currency)}</span></div>`);
	}
	...
	render_grand_total(value) {
		...
		this.$totals_section
			...
			.html(`<div>${__("Total à regler")}</div><div>${format_currency(value, currency)}</div>`);

		this.$numpad_section
			.find(".numpad-grand-total")
			// .html(`<div>${__("Grand Total")}: <span>${format_currency(value, currency)}</span></div>`);
			.html(`<div>${__("Total à regler")}: <span>${format_currency(value, currency)}</span></div>`);
	}
	...
	render_part_assure_total(value) {
		const currency = this.events.get_frm().doc.currency;
		this.$totals_section
			.find(".part-assure-total-container")
			.html(`<div>${__("Total part assuré")}</div><div>${format_currency(value, currency)}</div>`);
	
		this.$numpad_section
			.find(".numpad-part-assureur-total")
			.html(`<div>${__("Total part assuré")}: <span>${format_currency(value, currency)}</span></div>`);
	}

	render_part_assureur_total(value) {
		const currency = this.events.get_frm().doc.currency;
		this.$totals_section
			.find(".part-assureur-total-container")
			.html(`<div>${__("Total part assureur ")}</div><div>${format_currency(value, currency)}</div>`);
	
		this.$numpad_section
			.find(".numpad-part-assureur-total")
			.html(`<div>${__("Total part assureur")}: <span>${format_currency(value, currency)}</span></div>`);
	}
	...
	render_cart_item(item_data, $item_to_update) {
		...
		/* $item_to_update.html(
			...
			${get_rate_discount_html()}
			<div class="custom-part-assureur">
                ${item_data.custom_part_assureur}
            </div>`
		); */
		/* $item_to_update.html(
			`${get_item_image_html()}
		...
		function get_rate_discount_html() {
			if (item_data.rate && item_data.amount && item_data.rate !== item_data.amount) {
				// console.log('Custom Part Assureur:', item_data.custom_part_assureur);
				return `
					<div class="item-qty-rate">
						<div class="item-qty"style="margin-right: 1px !important;"><span>${item_data.qty || 0} ${item_data.uom}</span></div>
						<div class="item-rate-amount">
							<div class="item-rate">${format_currency(item_data.amount, currency)}</div>
							<div class="item-amount">${format_currency(item_data.rate, currency)}</div>
						</div>
						<div class="item-rate-amount"style="margin-left:8px; margin-right: 0px; text-align: left;">
							<div class="item-rate"style="margin-right: 1px; text-align: end;">${format_currency((item_data.custom_part_assuré * item_data.qty), currency)}</div>
							<div class="item-rate"style="margin-right: 1px; text-align: end;">${format_currency(item_data.custom_part_assuré, currency)}</div>
						</div>
					</div>`;
			} else {
				// console.log('Custom Part Assureur:', item_data.custom_part_assureur);
				return `
					<div class="item-qty-rate">
						...
						<div class="item-rate-amount" style="margin-left:8px;">
							<div class="item-rate"style="margin-right: 0px; text-align: end;">${format_currency(item_data.custom_part_assuré, currency)}</div>
						</div>
					</div>`;
			}
		}
		...
		function get_custom_part_assureur_html() {
			if (item_data.custom_part_assureur) {
				return `<div class="item-custom-part-assureur">${item_data.custom_part_assureur}</div>`;
			}è
			return ``;
		}
	}
		...
		$(frm.wrapper).on("refresh-fields", () => {
			if (frm.doc.items.length) {
				...
				frm.doc.items.forEach((item) => {
					...
					// console.log("refresh items: ",item);
				});
			}
			this.update_totals_section(frm);
		});
