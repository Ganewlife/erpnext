	...
	async on_cart_update(args) {
		...
		try {
			...
			if (item_row_exists) {
				...
				const { item_code, batch_no, serial_no, rate, uom, custom_part_assureur, custom_part_assuré } = item;

				if (!item_code) return;

				const new_item = { item_code, batch_no, rate, uom, custom_part_assureur, custom_part_assuré, [field]: value };
	...
	async on_cart_update(args) {
		...
		try {
			...
			if (item_row_exists) {
				...
				// Je fais le Fetch part asuré et part assureur fields 
				let response = await frappe.call({
					method: 'frappe.client.get_value',
					args: {
						doctype: 'Item',
						filters: { 'item_code': item_code },
						fieldname: ['custom_part_assureur', 'custom_part_assuré']
					}
				});
	
				const custom_part_assureur = response.message.custom_part_assureur || "";
				const custom_part_assuré = response.message.custom_part_assuré || "";
				...
				const new_item = {
					...
					uom,
					[field]: value,
					custom_part_assureur: custom_part_assureur,
					custom_part_assuré: custom_part_assuré,
				};
