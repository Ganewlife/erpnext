...
erpnext.PointOfSale.Payment = class {
				...
				.map((p, i) => {
					...
					<div class="payment-mode-wrapper">
						<div class="mode-of-payment" data-mode="${mode}" data-payment-type="${payment_type}">
							${p.mode_of_payment}
							<div class="${mode}-amount pay-amount">${amount} ad fifo</div>
	...
	render_payment_mode_dom() {
		...
		this.$payment_modes.html(
			`${payments
				.map((p, i) => {
					...
					const payment_type = p.type;
					const margin = i % 2 === 0 ? "pr-2" : "pl-2";
					const total_assure = frappe.model.get_value("POS Invoice", doc.name, "total_assuré");
					const amount = total_assure > 0 ? format_currency(total_assure, currency) : "";
		...
		payments.forEach((p) => {
			...
			const me = this;
			const total_assure = frappe.model.get_value("POS Invoice", doc.name, "total_assuré");
			const grand_total = frappe.model.get_value("POS Invoice", doc.name, "grand_total");
			...
			this[`${mode}_control`] = frappe.ui.form.make_control({
				df: {
					...
					onchange: function () {
						...
						if (current_value != this.value || total_assure != grand_total) {
							frappe.model
								.set_value(p.doctype, p.name, "amount", flt(this.value))
								.then(() => me.update_totals_section());
			...
			this[`${mode}_control`].toggle_label(false);
			this[`${mode}_control`].set_value(total_assure);
	
			// Check if total_assureur is different from grand_total and execute logic
			if (total_assure != grand_total) {
				const formatted_currency = format_currency(total_assure, currency);
				me.$payment_modes.find(`.${mode}-amount`).html(formatted_currency);
				frappe.model.set_value(p.doctype, p.name, "amount", total_assure).then(() => me.update_totals_section());
			}
		});
	...
	update_totals_section(doc) {
		...
		const label = change ? __("Change") : __("Dû par assureur");
