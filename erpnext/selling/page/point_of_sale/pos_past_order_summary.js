erpnext.PointOfSale.PastOrderSummary = class {
	...
	get_item_html(doc, item_data) {
		return `<div class="item-row-wrapper">
					...
					<div class="item-custom-part-assureur"style="margin-left: 15px;">${get_part_assure_html()}</div>
				</div>`;
		...
		function get_part_assure_html() {
			if (item_data.custom_part_assuré) {
				const part_assure_total = item_data.qty * item_data.custom_part_assuré;
				return `<span class="item-custom-part">${format_currency(part_assure_total, doc.currency)}</span>
						<div class="custom-part-total">${format_currency(item_data.custom_part_assuré, doc.currency)}</div>`;
			} else {
				return `<div class="custom-part-total">${format_currency(0, doc.currency)}</div>`;
			}
		}
	}
	...
	get_net_total_html(doc) {
		return `<div class="summary-row-wrapper">
					<div>${__("Net Total")}</div>
					<div>${format_currency(doc.total_assuré, doc.currency)}</div>
				</div>`;
	}
	...
	attach_totals_info(doc) {
		...
		const part_assureur_total_dom = this.get_part_assureur_total_html(doc);
		const part_assure_total_dom = this.get_part_assure_total_html(doc);
		const grand_total_dom = this.get_grand_total_html(doc);
		// this.$totals_container.append(net_total_dom);
		this.$totals_container.append(taxes_dom);
		this.$totals_container.append(discount_dom);
		this.$totals_container.append(part_assureur_total_dom);
		this.$totals_container.append(part_assure_total_dom);
		this.$totals_container.append(grand_total_dom);
	}
	get_part_assureur_total_html(doc) {
		const part_assureur_total = doc.items.reduce((total, item) => {
			return total + (item.qty * item.custom_part_assureur);
		}, 0);
	
		return `<div class="summary-row-wrapper">
					<div>${__("Total Net Part Assureur")}</div>
					<div>${format_currency(part_assureur_total, doc.currency)}</div>
				</div>`;
	}
	get_part_assure_total_html(doc) {
		const part_assure_total = doc.items.reduce((total, item) => {
			return total + (item.qty * item.custom_part_assuré);
		}, 0);
	
		return `<div class="summary-row-wrapper">
					<div>${__("Total Net Part Assuré")}</div>
					<div>${format_currency(part_assure_total, doc.currency)}</div>
				</div>`;
	}
